...
Resolution order:
1. If the sandbox has a known IP, use it directly (resolved via `container list --all --format json` on the Apple Container runtime)
2. Otherwise, look up the sandbox by name from the gateway's sandbox state
...
4. **vmnet network isolation** -- each sandbox VM gets its own network namespace via macOS vmnet, preventing lateral movement between sandboxes. The gateway process is the only host-side entity that connects to sandbox VMs on port 2222.

### Mandatory handshake secret

The NSSH1 handshake secret (`OPENSHELL_SSH_HANDSHAKE_SECRET`) is required. Both the server and sandbox will refuse to start if the secret is empty or unset. The secret is auto-generated by the bootstrap process via `openssl rand -hex 32` and passed to the server via CLI args.
